...
router.get('/finies', function(req, res, next) {
    ...
    pg.connect(databaseURL, function(err, client, done) {
        ...
        "WHERE date_fin IS NOT NULL ORDER BY date_debut DESC";
        client.query(sql, function (err, locations_get){
        // Erreurs
...
router.get('/finies/:id', function(req, res, next) {
    ...
    if (id != undefined) {
        ...
        "WHERE idlocation=($1) AND date_fin IS NOT NULL";

            client.query(sql, [id], function (err, locationsid_get){
                if (err) {
                    res.status(401);
...
router.get('/encours', function(req, res, next) {
    ...
    pg.connect(databaseURL, function(err, client, done) {
        ...
        "FROM locations "+
        "INNER JOIN clients ON locations.client_location = clients.idclient "+
        "WHERE date_fin IS NULL ORDER BY date_debut DESC";
        client.query(sql, function (err, locations_get){
        // Erreurs
            if (err) return next(err);
            done();
...
router.get('/encours/:id', function(req, res, next) {
    ...
    if (id != undefined) {
        ...
        "position_debut, station_debut, date_debut, position_fin, station_fin, date_fin "+
        "FROM locations "+
        "WHERE idlocation=($1) AND date_fin IS NULL";
...
/* GET /locations/domaine/iddomaine */
router.get('/domaine/:id', function(req, res, next) {
    var id = req.params.id;
    if (id != undefined) {
        pg.connect(databaseURL, function(err, client, done) {

            client.query("SELECT * FROM locations  WHERE dom_location=($1)", [id], function (err, locationsid_get){
                if (err) {
                    res.status(401);
                    return res.json({"status": 401, "message": err});
                }
...
router.get('/client/:id', function(req, res, next) {
    ...
    if (id != undefined) {
        ...
        "WHERE client_location=($1) ORDER BY date_debut DESC";

            client.query(sql, [id], function (err, locationsCLIENTid_get){
                if (err) {
                    res.status(401);
                    return res.json({"status": 401, "message": err});
                }
...
router.get('/velo/:id', function(req, res, next) {
    ...
    if (id != undefined) {
        ...
        var sql =  "SELECT * FROM locations "+
        "WHERE velo_location=($1) ORDER BY date_debut DESC";

            client.query(sql, [id], function (err, locationsVELOid_get){
                if (err) {
                    res.status(401);
                    return res.json({"status": 401, "message": err});
                }
...
https://localhost:80/api/expl/locations/finies get
[
    {
        "idlocation": 1,
        "dom_location": "test",
...
https://localhost:80/api/expl/locations/finies/1 get
[
    {
        "idlocation": 1,
        "dom_location": "test",
...
https://localhost:80/api/expl/locations/encours get
...
https://localhost:80/api/expl/locations/encours/3 get
