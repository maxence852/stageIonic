...
router.get('/', function(req, res, next) {
    ...
    pg.connect(databaseURL, function(err, client, done) {
        ...
        "WHERE dom_transaction=($1) ORDER BY date_transaction DESC";
        client.query(sql, [dom], function (err, transactions_get){
        // Erreurs
...
router.get('/:id', function(req, res, next) {
    ...
    if (id != undefined) {
        ...
        "INNER JOIN clients ON transactions.client_transaction = clients.idclient "+
        "LEFT JOIN admins ON transactions.auteur = admins.idadmin "+
        "WHERE idtransaction=($1) AND dom_transaction=($2)";

            client.query(sql, [id, dom], function (err, transactionsid_get){
                if (err) {
                    res.status(401);
...
router.get('/client/:id', function(req, res, next) {
    ...
    if (id != undefined) {
        ...
        "INNER JOIN clients ON transactions.client_transaction = clients.idclient "+
        "LEFT JOIN admins ON transactions.auteur = admins.idadmin "+
        "WHERE client_transaction=($1) AND dom_transaction=($2) ORDER BY date_transaction DESC";
...
https://localhost:80/api/oper/transactions get
[
    {
        "idtransaction": 1,
...
https://localhost:80/api/oper/transactions/1 get
[
    {
        "idtransaction": 1,
        "dom_transaction": "test",
...
https://localhost:80/api/oper/transactions  post
